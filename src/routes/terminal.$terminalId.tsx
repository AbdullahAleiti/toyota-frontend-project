...
function TerminalLogin() {
  ...
  const onSubmit: SubmitHandler<TerminalLoginForm> = async (data) => {
    try {
      ...
      const response = await axios.post<TerminalLoginForm>(
        "https://api.sunucu.com/login/",
        data
      );
  ...
  const options = useMemo(
    () =>
      terminalData.map<Options>((terminal) => ({
        value: terminal.termName,
        label: terminal.termName,
      })),
    [terminalData]
  );
  ...
  return (
    <form
      className="mx-auto min-w-100 w-1/3 mt-10"
